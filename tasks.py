...
SOURCE_DIR = ROOT_DIR.joinpath("pylabrobot_sila_bridge")
...
PYTHON_VERSION = 3.9
CI_PROJECT_NAME = "pylabrobot-sila-bridge"
CI_REGISTRY_IMAGE = "registry.gitlab.com/https://gitlab.com/opensourcelab/pylabrobot-sila-bridge"
...
def docker_test(_c, python_version=PYTHON_VERSION):
    ...
    """
    volume_mount = (
        f"--volume {BIN_DIR}:/pylabrobot_sila_bridge/bin/ --volume {ROOT_DIR}:/pylabrobot_sila_bridge:rw"
    )
    registry_path = _get_registry_path_str(python_version)
    pytest_arg = f"pytest -v --cov-report xml:/pylabrobot_sila_bridge/bin/coverage.xml {TEST_DIR}"
    Path(BIN_DIR).mkdir(parents=True, exist_ok=True)
